...
def reward(pacmanx:int,pacmany:int,ghostx:list,ghosty:list,ghostnum:int):
    ...
    for i in range(ghostnum):
        ...
        if pacmanx==ghostx[i] and pacmany==ghosty[i]:
            ghostdist[i]=0
            continue
        tempway=pathctrl.how_to_go(ghostx[i],ghosty[i],pacmanx,pacmany)
        if tempway==None:
    ...
    if Map.mapobject.get_ifeaten(pacmanx,pacmany)==1:
        ...
        scafood=pathctrl.find_nearest(pacmanx,pacmany)
        for ele in scafood:
            if Map.mapobject.get_ifcapsule(ele[0],ele[1])==True:
                capsuledis=min(capsuledis,len(pathctrl.how_to_go(pacmanx,pacmany,ele[0],ele[1]))-1)
                fooddis=capsuledis
            else:
                fooddis=min(fooddis,len(pathctrl.how_to_go(pacmanx,pacmany,ele[0],ele[1]))-1)
...
def max_search(depth,pacmanx:int,pacmany:int,ghostx:list,ghosty:list,ghostnum:int):
    ...
    if depth>=2:
        ...
        return (None,reward(pacmanx,pacmany,ghostx,ghosty,ghostnum))
    for i in range(4):
        newx= pacmanx + Direction.movevector[i][0]
        newy= pacmany + Direction.movevector[i][1]
...
def min_search(depth,pacmanx:int,pacmany:int,ghostx:list,ghosty:list,ghostnum:int,nowghost:int):
    ...
    for i in range(4):
        newghostx= ghostx[nowghost] + Direction.movevector[i][0]
        newghosty= ghosty[nowghost] + Direction.movevector[i][1]
    ...
    for i in range(5):
        for j in range(5):
            ...
            if ghostnum<=0:
                ...
                newghostx[0]=0
            if i!=4:
                newghostx[0]= newghostx[0] + Direction.movevector[i][0]
                newghosty[0]= newghosty[0] + Direction.movevector[i][1]
                
            if ghostnum<=1:
                if j>0:
                    continue
            elif j!=4:
                newghostx[1]= newghostx[1] + Direction.movevector[j][0]
                newghosty[1]= newghosty[1] + Direction.movevector[j][1]

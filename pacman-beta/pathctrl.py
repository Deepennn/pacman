...
def dfs_shortest_origin(nowx, nowy):
    ...
    if tempmap.get_ifeaten(nowx,nowy)==1:
        ...
        for i in range(4):
            tempvector=Direction.getmove(i)
    ...
    while len(temp_BFSqueue)>0:
        ...
        if tempmap.get_ifeaten(temp_x,temp_y)==1:
            while True:
                ...
                tempvector=Direction.getmove(temp_direction)
...
def greedy_search_dfs():
    global dfs_visitmap,dfs_goal
    tempsize=Map.mapobject.get_size()
    tempdic=Pacman.pacmanobject.getposition()
    if Map.mapobject.if_win()==True:
        return -1
    if dfs_goal==None or (tempdic[0] == dfs_goal[0] and tempdic[1] == dfs_goal[1]):
        ...
        temp_way=how_to_go(tempdic[0], tempdic[1], dfs_goal[0], dfs_goal[1])
        return temp_way[1][2]
